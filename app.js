...
function ContactListController($scope, ContactService) {
  $scope.ContactList = ContactService.getContactList().map(function(x){return x[0]+" "+x[1]; }).sort();

  $scope.setContact = function(item){
    ContactService.setContact(item);
  }
}

ShowContactController.$inject = ['ContactService'];
function ShowContactController(ContactService) {
  var scope=this;
  scope.showContact = ContactService.getContact();
...
function ContactService(){
  ...
  service.setContact= function(item){
    contact.pop(12);
    var x = item.split(" ")[0];
    var flat = [].concat.apply([], ContactList);
    var col = flat.indexOf(x);
    col=col/12;

    contact.push(ContactList[col]);
  }
